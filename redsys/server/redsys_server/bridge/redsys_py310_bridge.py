...
def _load_util_module(runtime_dir: Path, request: dict[str, str]):
    ...
    # Redsys 的 32 位 DLL 还依赖安装目录中的 AxTpvpcPinPadWS.dll。
    # 仅加载内部 dllTpvpcLatente.dll 不足以完成初始化。
    dependency_dirs = request.get("dependency_dirs") or []
    if isinstance(dependency_dirs, str):
        dependency_dirs = [dependency_dirs]
    for dependency_dir in dependency_dirs:
        path = Path(str(dependency_dir)).resolve()
        if path.exists():
